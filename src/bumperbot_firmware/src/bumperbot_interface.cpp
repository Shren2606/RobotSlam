...
CallbackReturn BumperbotInterface::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  try
  {
    ...
    RCLCPP_WARN(rclcpp::get_logger("BumperbotInterface"),
                "Waiting for Arduino to reset and respond...");
    ...
    while (std::chrono::steady_clock::now() - start_time < std::chrono::seconds(5))
    {
      if (arduino_.IsDataAvailable())
      {
        ...
        RCLCPP_WARN(rclcpp::get_logger("BumperbotInterface"),
                "[BOOT] Arduino says: %s", line.c_str());  // Màu vàng
    ...
    if (!ready_received)
    {
      RCLCPP_ERROR(rclcpp::get_logger("BumperbotInterface"),
                   "Timeout waiting for Arduino to send 'ready'.");
      return CallbackReturn::FAILURE;
    }
  }
  catch (...)
                                                          ...
                                                          const rclcpp::Duration &)
{
  ...
  if(arduino_.IsDataAvailable())
  {
    ...
    //======debug=============
    RCLCPP_INFO(rclcpp::get_logger("BumperbotInterface"),
            "[RX] Received from Arduino: %s", message.c_str());
                                                          ...
                                                          const rclcpp::Duration &)
{
  ...
  try
  {
    //std::string msg = "rp25,lp00,";
    if (!arduino_.IsOpen()) return hardware_interface::return_type::ERROR;

    arduino_.Write(message_stream.str());  // thêm "\n"

    //arduino_.Write(msg);

    //======debug=============
    // RCLCPP_INFO(rclcpp::get_logger("BumperbotInterface"),
    //         "[TX] Sent to Arduino: %s", message_stream.str().c_str());
    //  RCLCPP_INFO(rclcpp::get_logger("BumperbotInterface"),
    //         "[TX] Sent to Arduino: %s", msg.c_str());
  }
  catch (...)
  {
